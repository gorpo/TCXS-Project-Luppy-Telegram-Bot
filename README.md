...
8. ATENÇÃO: insira no arquivo "config.py" os tokens requeridos do bot, dropbox,imgur,yandex,giphy e também as id's de canal, grupo, admins, sudos(mesmo que sudo do linux, necessario id dos admins), tudo esta comentado e tem os links das API's para pegar as id's no codigo.  
